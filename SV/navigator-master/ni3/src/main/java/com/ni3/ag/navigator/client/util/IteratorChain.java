 ...
 * <p>
 * This class makes multiple iterators look like one to the caller When any method from the Iterator interface is
 * called, the IteratorChain will delegate to a single underlying Iterator. The IteratorChain will invoke the Iterators
 * in sequence until all Iterators are exhausted.
 * <p>
 * Under many circumstances, linking Iterators together in this manner is more efficient (and convenient) than reading
 * out the contents of each Iterator into a List and creating a new Iterator.
 * <p>
 * Calling a method that adds new Iterator <i>after a method in the Iterator interface has been called </i> will result
 * in an UnsupportedOperationException. Subclasses should <i>take care </i> to not alter the underlying List of
 * Iterators.
 * <p>
 * NOTE: As from version 3.0, the IteratorChain may contain no iterators. In this case the class will function as an
 * empty iterator.
 * 
 ...
 */
@Deprecated // remove with CompositeCollection
...
public class IteratorChain implements Iterator{
	...
	/**
	 * The "last used" Iterator is the Iterator upon which next() or hasNext() was most recently called used for the
	...
	/**
	 * Determine if modifications can still be made to the IteratorChain. IteratorChains cannot be modified once they
	 * have executed a method from the Iterator interface.
	 * 
	...
	/**
	 * Removes from the underlying collection the last element returned by the Iterator. As with next() and hasNext(),
	 * this method calls remove() on the underlying Iterator. Therefore, this method may throw an
	 * UnsupportedOperationException if the underlying Iterator does not support this method.
	 * 
	 * @throws UnsupportedOperationException
	 *             if the remove operator is not supported by the underlying Iterator
	 * @throws IllegalStateException
	 *             if the next method has not yet been called, or the remove method has already been called after the
