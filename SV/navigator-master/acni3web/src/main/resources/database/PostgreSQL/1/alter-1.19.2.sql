...
CREATE OR REPLACE FUNCTION alterDatabase()
 RETURNS void AS $$
DECLARE 
	_expectedVersion varchar = '1.19.1';
	_newVersion varchar = '1.19.2';
	_version varchar;
BEGIN
	-- check version
...
$BODY$
	...
	DropTableColumns cursor is 
        ...
        where not exists(select *
              ...
              and lower(col.column_name) not like '%_error'
	...
	from cht_datatype phys_dt inner join SYS_OBJECT_Attributes oa on phys_dt.id = oa.dbdatatypeId 
			inner join SYS_OBJECT_Definition od on oa.ObjectDefinitionID=od.id
	where exists(select *
		from information_schema.tables tbl inner join information_schema.columns col on
			(tbl.table_catalog=col.table_catalog and tbl.table_schema=col.table_schema and tbl.table_name=col.table_name)
		where tbl.table_name iLike replace(COALESCE(oa.inTable,od.TableName),'dbo.','') 
		    and col.column_name iLike oa.name
		    and not (
...
BEGIN
    ...
    begin
		...
		exception
		when OTHERS then
			_SQL := 'ALTER TABLE '||_tablename||' RENAME COLUMN '||_columnname||' TO '||_columnname||'_error';
			raise info '%',_SQL;
			execute _SQL;
