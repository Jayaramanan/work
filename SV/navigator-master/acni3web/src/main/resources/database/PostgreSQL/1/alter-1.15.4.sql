...
CREATE OR REPLACE FUNCTION alterDatabase()
 RETURNS void AS $$
DECLARE 
	_expectedVersion varchar = '1.15.3';
	_newVersion varchar = '1.15.4';
	_version varchar;
BEGIN
	-- check version
...
------------------------------------------------------------
alter table sys_object_attributes add column length integer;

update sys_object_attributes set length = 255 where length is null and datatypeid = 1;
...
$BODY$
	...
	select distinct 'ALTER TABLE '||replace(COALESCE(t.inTable,d.TableName),'dbo.','')||' ADD '||t.name||case when t.predefined=1 or t.datatypeid in (2,4) then ' numeric NULL' else  case when t.length is null then ' varchar(255) NULL' else (' varchar('||t.length||')') end end
	...
	DropTableColumns cursor is 
        ...
        where not exists(select *
                ...
                where o.table_name  iLike  replace(COALESCE(t.inTable,d.TableName),'dbo.','') 
                        and a.column_name  iLike t.name 
                        and (
                        	(a.udt_name like 'varchar' and t.datatypeId not in (2,4) and 
					t.length = a.character_maximum_length)
                        	or 
