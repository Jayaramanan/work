...
CREATE OR REPLACE FUNCTION alterDatabase()
 RETURNS void AS $$
DECLARE 
	_expectedVersion varchar = '1.19.3';
	_newVersion varchar = '1.19.4';
	_version varchar;
BEGIN
	-- check version
...
------------------------------------------------------------
CREATE TABLE sys_user_data_error
(
  id serial NOT NULL,
  errorid integer,
  ...
  "value" text,
  error text,
  errtime timestamp without time zone,
  CONSTRAINT sys_user_data_error_pk PRIMARY KEY (id)
)
WITH (
  OIDS=FALSE
);
...
$BODY$
	...
	inner join information_schema.columns col on (tbl.table_catalog=col.table_catalog and tbl.table_schema=col.table_schema and tbl.table_name=col.table_name)
	where col.column_name iLike oa.name
	    and not(
