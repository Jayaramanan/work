...
import com.ni3.ag.navigator.server.services.mock.ExportServiceMock;
...
public class UserDataExporterTest extends TestCase{
	...
	protected void setUp() throws Exception{
		...
		exporter = new UserDataExporter(new ExportServiceMock());
		data = new ArrayList<Object[]>();
	}

	public void testParseFromToIds(){
		...
		exporter.parseFromToIDs(data, attributes);
	...
	public void testPrepareData(){
		...
		exporter.setDefaultDateFormat(exporter.getDateFormat(DataType.DISPLAY_DATE_FORMAT));
	...
	public void testGetFormattedDate(){
		exporter.setDefaultDateFormat(exporter.getDateFormat(DataType.DISPLAY_DATE_FORMAT));
		DateFormat format = exporter.getDateFormat("dd/MM/yyyy");
		assertEquals("20/10/2010", exporter.getFormattedDate("20101020", format));
		assertEquals("02/01/2000", exporter.getFormattedDate("20000102", format));
		assertEquals("", exporter.getFormattedDate("20013", format));
		assertEquals("", exporter.getFormattedDate(null, format));
	}
	...
	public void testPrepareMultivalue(){
		assertNull(exporter.prepareMultivalue(null));
		assertEquals("", exporter.prepareMultivalue(""));
		assertEquals("1;12;123", exporter.prepareMultivalue("{1}{12}{123}"));
		assertEquals("123", exporter.prepareMultivalue("{123}"));
	}

	public void testGetObjectDefinitions(){
		List<ObjectDefinition> objectDefinitions = exporter.getObjectDefinitions("", "");
		assertEquals(0, objectDefinitions.size());
		objectDefinitions = exporter.getObjectDefinitions("123,456", "321");
		assertEquals(2, objectDefinitions.size());
	}

	public void testFormatNumber(){
		final Integer intg = new Integer(12);
		assertEquals(intg, exporter.formatNumber(12));
		assertEquals(intg, exporter.formatNumber(new Integer(12)));
		assertEquals(intg, exporter.formatNumber(new Short("12")));
		assertEquals(intg, exporter.formatNumber(new Long(12)));
		assertEquals(intg, exporter.formatNumber(new BigInteger("12")));

		final Double dbl = new Double(12.345);
		assertEquals(dbl, exporter.formatNumber(12.345));
		assertEquals(dbl, exporter.formatNumber(new Double(12.345)));
		assertEquals(dbl, exporter.formatNumber(new BigDecimal("12.345")));
		assertEquals(dbl, exporter.formatNumber(new Float("12.345")));

		assertEquals(new Double(12), exporter.formatNumber(new Double(12)));
		assertEquals(new Double(12), exporter.formatNumber(new BigDecimal("12")));
		assertEquals(new Double(12), exporter.formatNumber(new Float("12")));
	}
