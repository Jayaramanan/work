 ...
 * @author Mark Proctor
 */
@Deprecated // remove, replace with HashMap
@SuppressWarnings("unchecked")
public class PrimitiveIntMap implements Serializable{
	/**
     * 
     */
	...
	public PrimitiveIntMap(){
		this(32, 8);
	}

	public PrimitiveIntMap(final int tableSize){
		this(tableSize, 8);
	}

	public PrimitiveIntMap(final int tableSize, final int indexIntervals){
	...
	public Object put(final int key, Object value){
		if (key < 0){
	...
	public Object get(final long key){
		...
		if (PrimitiveIntMap.NULL.equals(value)){
			value = null;
		}
		return value;
	}
	...
	private Page findPage(final long key){
		...
		// if pageId is greater than lastTopNodeId need to expand
			...
			// are we before or after the halfway point of an index
			// interval
			if ((offset != (this.pageIndex.length - 1))
			        && ((key - (offset << this.intervalShifts << this.doubleShifts)) > this.midIntervalPoint)){
	 ...
	 */
		...
		Page(final Page previousSibling, final int pageId, final int tableSize){
		...
		public Object put(long key, final Object newValue){
			if (this.tables == null){
				// initiate tree;
