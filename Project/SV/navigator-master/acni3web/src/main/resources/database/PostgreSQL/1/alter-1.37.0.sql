...
CREATE OR REPLACE FUNCTION alterDatabase()
 RETURNS void AS $$
DECLARE 
	_expectedVersion varchar = '1.36.2';
	_newVersion varchar = '1.37.0';
	_version varchar;
	_cc integer;
BEGIN
	-- check version
...
-- Function: getchart(integer, integer, text, text)

-- DROP FUNCTION getchart(integer, integer, text, text);
...
BEGIN
	...
	_mSelect:='select n.id, COALESCE(cast(((case when('||_inSQL||')<ch.minvalue then ch.minvalue when('||_inSQL||')>ch.maxvalue then ch.maxvalue else('||_inSQL||') end)-ch.minvalue)/(ch.maxvalue-ch.minvalue)*(ch.maxscale-ch.minscale)+ch.minscale AS integer),ch.minvalue), ch.rgb,ch.isvaluedisplayed, ch.LabelInUse,ch.LabelFontSize,ch.NumberFormat ';
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION getchart(integer, integer, text, text) OWNER TO sa;
