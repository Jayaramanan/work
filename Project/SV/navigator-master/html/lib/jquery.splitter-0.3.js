...
(function ($, undefined) {
    ...
    $.fn.split = function (options) {
                ...
                if (settings.orientation == 'vertical') {
                    return function (n) {
                        if (n === undefined) {
                            ...
                            position = n;
                            var sw = spliter.width() * settings.initial_position;
                    ...
                    return function (n) {
                        if (n === undefined) {
                            ...
                            var sw = spliter.height() * settings.initial_position;
        ...
        if (settings.orientation == 'vertical') {
            position = width * settings.initial_position;
        } else if (settings.orientation == 'horizontal') {
            position = height * settings.initial_position;
        }
        ...
        if (spliters.length == 0) { // first time bind events to document
            ...
            $(window).resize(function () { // resize second panel (right or bottom)
                for (var s_id in spliters) {
                    var spl = spliters[s_id];
                    var limit = spl.limit;
                    var pos = spl.position();
                    if (spl.orientation == 'vertical') {
                        if (pos > spl.width() - limit) {
                            pos = spl.width() - limit;
                        }
                    } else if (spl.orientation == 'horizontal') {
                        if (pos > spl.height() - limit) {
                            pos = spl.height() - limit;
                        }
                    }
                }
                spl.position(pos);
                spl.find('.spliter_panel').trigger('spliter.resize');
            });
        }
        spliters.push(self);
        return self;
